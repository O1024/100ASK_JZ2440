...
int hal_eth_rx(uint8_t *buffer) {
    ...
    if (flag != 0x01 || (status & RSR_ERR_BITS) || length > 1536) {
        hal_uart_puts("[ETH] RX Packet Error. Dropping...\r\n");
...
void hal_eth_dump_status(void) {
    ...
    uint8_t ncr = eth_read_reg(REG_NCR);
    uint8_t nsr = eth_read_reg(REG_NSR);
    hal_uart_puts("[ETH] NCR:");
    hal_uart_putc(hex[ncr >> 4]); hal_uart_putc(hex[ncr & 0xF]);
    hal_uart_puts(" NSR:");
    hal_uart_putc(hex[nsr >> 4]); hal_uart_putc(hex[nsr & 0xF]);
    hal_uart_puts("\r\n");
}
